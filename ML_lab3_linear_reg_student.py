...
def parsePoint(line):
    ...
    """
    sl = line.split(',')
    point = LabeledPoint(sl[0], sl[1:])
    return point

parsedSamplePoints = [parsePoint(sp) for sp in samplePoints]
...
# TODO: Replace <FILL IN> with appropriate code
parsedDataInit = rawData.map(lambda rd: parsePoint(rd))
onlyLabels = parsedDataInit.map(lambda pd: pd.label)
...
# TODO: Replace <FILL IN> with appropriate code
parsedData = parsedDataInit.map(lambda pd: LabeledPoint((pd.label - minYear), pd.features))
...
def calcRMSE(labelsAndPreds):
    ...
    """
    length = labelsAndPreds.count()
    return ((labelsAndPreds.map(lambda (label, prediction): squaredError(label, prediction)).reduce(lambda a, b: a+b))/length) ** 0.5
...
# TODO: Replace <FILL IN> with appropriate code
labelsAndPredsTrain = parsedTrainData.map(lambda ptd: (ptd.label, averageTrainYear))
rmseTrainBase = calcRMSE(labelsAndPredsTrain)

labelsAndPredsVal = parsedValData.map(lambda ptd: (ptd.label, averageTrainYear))
rmseValBase = calcRMSE(labelsAndPredsVal)

labelsAndPredsTest = parsedTestData.map(lambda ptd: (ptd.label, averageTrainYear))
...
def linregGradientDescent(trainData, numIters):
    ...
    for i in range(numIters):
        ...
        labelsAndPredsTrain = trainData.map(lambda td: getLabeledPrediction(w, td))
        
        errorTrain[i] = calcRMSE(labelsAndPredsTrain)
        
        # Calculate the `gradient`.  Make use of the `gradientSummand` function you wrote in (3a).
        # Note that `gradient` sould be a `DenseVector` of length `d`.
        gradient = sum(trainData.map(lambda lp: gradientSummand(w, lp)).collect())
...
labelsAndPreds = parsedValData.map(lambda pd: getLabeledPrediction(weightsLR0, pd))
...
# TODO: Replace <FILL IN> with appropriate code
firstModel = LinearRegressionWithSGD.train(parsedTrainData, iterations=numIters, step=alpha, regParam=reg, regType=regType, intercept=useIntercept, miniBatchFraction=miniBatchFrac)
...
# TODO: Replace <FILL IN> with appropriate code
labelsAndPreds = parsedValData.map(lambda pvd: (pvd.label, firstModel.predict(pvd.features)))
...
def twoWayInteractions(lp):
    ...
    """
    features = [d[0]*d[1] for d in list(itertools.product(lp.features, repeat=2))]
    complete_feautres = np.hstack((lp.features, features))
    return LabeledPoint(lp.label, complete_feautres)

print twoWayInteractions(LabeledPoint(0.0, [2, 3]))

# Transform the existing train, validation, and test sets to include two-way interactions.
trainDataInteract = parsedTrainData.map(lambda td: twoWayInteractions(td))
valDataInteract = parsedValData.map(lambda td: twoWayInteractions(td))
testDataInteract = parsedTestData.map(lambda td: twoWayInteractions(td))
...
# TODO: Replace <FILL IN> with appropriate code
labelsAndPredsTest = testDataInteract.map(lambda tdi: (tdi.label, modelInteract.predict(tdi.features)))
