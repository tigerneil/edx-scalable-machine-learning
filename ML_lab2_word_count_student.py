...
# TODO: Replace <FILL IN> with appropriate code
pluralLambdaRDD = wordsRDD.map(lambda rec: rec + 's' )
...
# TODO: Replace <FILL IN> with appropriate code
wordPairs = wordsRDD.map(lambda rec: (rec, 1))
...
# TODO: Replace <FILL IN> with appropriate code
uniqueWords = wordsRDD.map(lambda x: x).distinct().count()
print uniqueWords
...
def removePunctuation(text):
    ...
    regex = re.compile('[%s]|^\s+|\s+$'%re.escape(string.punctuation))
...
# TODO: Replace <FILL IN> with appropriate code
# shakespeareWordsRDD = shakespeareRDD.filter(lambda x: len(x)>0).flatMap(lambda x: x.split()).map(lambda x: (x, 1)).map(lambda x:(x[0]))
...
# TODO: Replace <FILL IN> with appropriate code
top15WordsAndCounts = wordCount(shakeWordsRDD).takeOrdered(15, key=(lambda w: -w[1]))
